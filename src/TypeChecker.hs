...
tc (BinaryOp p u t1 t2) bs          = do ty1 <- tc t1 bs
                                         expect NatTy ty1 t1
                                         ty2 <- tc t2 bs
                                         expect NatTy ty2 t2
tc (IfZ p c t t') bs                = do tyc  <- tc c bs
                                         expect NatTy tyc c
                                         tyt  <- tc t bs
                                         tyt' <- tc t' bs
                                         expect tyt tyt' t'
tc (Lam p v (NTy n ty) t) bs        = tc (Lam p v ty t) bs
tc (Lam p v ty t) bs                = do ty' <- tc (open v t) ((v,ty):bs)
                                         return (FunTy ty ty')
tc (App p t u) bs                   = do tyt       <- tc t bs
...
tc (Fix p f fty x xty t) bs         = do 
      ...
      when (not (eq dom xty)) $ do failPosPCF p "El tipo del argumento de un fixpoint debe coincidir con el \
